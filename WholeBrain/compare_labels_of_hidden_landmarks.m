function score = compare_labels_of_hidden_landmarks(curr_labels,rand_selection,landmark_to_neuron_map,landmark_names,thisimage_r,mu_r,Neuron_head,conserved_nodeBel)

correct = 0;
cnt = 0;
for i = 1:size(landmark_to_neuron_map)
    if isempty(find(i == rand_selection))
        landmark_label = landmark_names{i,1};
        assigned_label = Neuron_head{curr_labels(landmark_to_neuron_map(i,1),1),1};
