function [score,not_match] = compare_labels_of_hidden_landmarks(curr_labels,marker_to_neuron_map,marker_name,landmarks,Neuron_head)

correct = 0;
cnt = 0;
not_match = [];
for i = 1:size(marker_name,1)
    if ~strcmp(marker_name{i,1},'RIGR') && ~strcmp(marker_name{i,1},'AVG')
        landmark_label = marker_name{i,1};
        assigned_label = Neuron_head{curr_labels(i,1),1};
        if strcmp(landmark_label,assigned_label)
            correct = correct + 1;
        else
            not_match = [not_match;i];
        end
        cnt = cnt + 1;
    end
