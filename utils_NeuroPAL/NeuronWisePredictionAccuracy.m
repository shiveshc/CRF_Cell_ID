...
figure,scatter3(annotated_neurons_pos_atlas(:,1),annotated_neurons_pos_atlas(:,2),-annotated_neurons_pos_atlas(:,3),100,RelPos_accuracy - Reg_3D_accuracy,'.')
hold on
for i = 1:size(Neuron_head,1)
    if isempty(find(strcmp(Neuron_head{i,1},uniq_annotated_neurons)))
        scatter3(X_rot(i,1),Y_rot(i,1),-Z_rot(i,1),100,[0.8,0.8,0.8],'.')
    end
end
for i = 1:size(annotated_neurons_pos_atlas,1)
    text(annotated_neurons_pos_atlas(i,1) + 2,annotated_neurons_pos_atlas(i,2) + 2, -annotated_neurons_pos_atlas(i,3) - 2,uniq_annotated_neurons{i,1},'FontSize',7)
end
map = winter(5);
colormap(jet)
caxis([-0.5,0.5])
