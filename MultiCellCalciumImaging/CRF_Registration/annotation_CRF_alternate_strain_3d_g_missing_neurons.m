...
function [landmark_match_score,not_match] = annotation_CRF_alternate_strain_3d_g_missing_neurons(data,rng_missing_neurons,num_missing_neurons,numLabelRemove)
rng(rng_missing_neurons)
%%% load image data and neuron relationship data (don't forget
%%% landmark_names and landmark_to_neuron_map
load(['C:\Users\Shivesh\Dropbox (GaTech)\PhD\GlobalBrainCode\GlobalBrainTrackingNew\functions\Annotation_CRF\NeuroPAL_validation\prediction_alternateStrains\AML5\RelPos_Reg\',data,'.mat'])
...
index_neurons_missing = randperm(size(mu_marker,1),num_missing_neurons);
X(index_neurons_missing,:) = [];
Y(index_neurons_missing,:) = [];
Z(index_neurons_missing,:) = [];
X_norm(index_neurons_missing,:) = [];
geo_dist_r(index_neurons_missing,:) = [];
geo_dist_r(:,index_neurons_missing) = [];
...
% experiments_file = ['/gpfs/pace1/project/pchbe2/schaudhary9/Annotation/NeuroPAL/AlternateStrains/RelPos/Results_multi_3D/experiments_',data,'_',num2str(numLandmarks),'_',num2str(numLabelRemove),'_',num2str(rng_fix_landmark),'_',num2str(num),'.mat'];
