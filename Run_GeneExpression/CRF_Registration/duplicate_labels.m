function node_label = duplicate_labels(curr_labels,X,Y,Z,PA_matrix,LR_matrix,DV_matrix,geo_dist,geo_dist_r,lambda_geo,node_pot,clamped_neurons)
    ...
    for i = 1:size(curr_labels,1)
        if node_label(i,2) == 1
            node_label(i,3) = consistency_score(i,node_label(i,1),node_label,X,Y,Z,PA_matrix,LR_matrix,DV_matrix,geo_dist,geo_dist_r,lambda_geo,node_pot);
        end
    end
...
function score = consistency_score(i,i_label,node_label,X,Y,Z,PA_matrix,LR_matrix,DV_matrix,geo_dist,geo_dist_r,lambda_geo,node_pot)
    reg_consistentcy_score = node_pot(i,i_label);
    ...
    score = (PA_consistency_score + LR_consistency_score + DV_consistency_score + geodist_consistency_score)/(4*max_consistency_score) + + reg_consistentcy_score;
end